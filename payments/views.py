...
#Muti tasks:
from django_ecommerce.celery import addtest
from django_ecommerce.celery import run
from django_ecommerce.celery import runemail
from django_ecommerce.celery import gmail_sender
from django_ecommerce.celery import send_email
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			try:
				...
				#run.delay((2,2), user.email)
				#addtest.delay(3,5)
				#run.delay(user.email)
				#runemail.delay(user.email)
				#gmail_sender.delay()
